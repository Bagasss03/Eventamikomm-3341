...
<main class="max-w-7xl mx-auto px-6 py-12 grid grid-cols-1 lg:grid-cols-3 gap-12">
    ...
    <div class="lg:col-span-1">
        <div class="sticky top-32">
            <img src="{{ ($event->poster_path && Storage::disk('public')->exists($event->poster_path)) ? asset('storage/' . $event->poster_path) : 'https://placehold.co/400x400?text=' . urlencode($event->title) }}" alt="{{ $event->title }}"
                class="w-full rounded-[2.5rem] shadow-2xl border-8 border-white">
            <div class="mt-8 p-6 bg-white rounded-3xl border border-slate-100 shadow-sm">
                <h4 class="font-bold mb-4">Detail Event</h4>
                <div class="space-y-3 text-sm text-slate-600">
                    <p><span class="font-bold text-slate-800">Kategori:</span> {{ $event->category->name ?? '-' }}</p>
                    <p><span class="font-bold text-slate-800">Stok Tiket:</span> {{ $event->stock }} tersisa</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <div
            ...
            <div class="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
                ...
                <div>
                    ...
                    @else
                    <span class="inline-block px-10 py-5 bg-slate-300 text-slate-500 rounded-2xl font-black text-xl cursor-not-allowed">
                        Tiket Habis
                    </span>
                    @endif
                </div>
